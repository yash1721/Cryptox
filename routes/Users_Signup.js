...
async function handlePostRequest(req, res, userData) {
    ...
    pythonProcess.stdin.write(userData.imageencoding);
    ...
    pythonProcess.stderr.on('data', (data) => {
        return res.status(401).json({
            success: false,
            message: "Aadhar Card not Verified Successfully"
        });
    });

    pythonProcess.on('close', async (code) => {
        if (pythonOutput != userData.aadharNumber) {
            console.log('hi');
            return res.status(403).json({
                success: false,
                message: "Aadhar Card number Not Matching"
            });
        }
